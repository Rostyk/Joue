...
+ (NSString *)serviceURL
{
    NSString *basicURL = [MTJoueSyncManager sharedManager].URL;
    return [NSString stringWithFormat:@"%@/wc-api/v2", basicURL];
}

+ (NSString *)postsServiceURL
{
    NSString *basicURL = [MTJoueSyncManager sharedManager].URL;
    return [NSString stringWithFormat:@"%@/wp-json/wp/v2", basicURL];
}
