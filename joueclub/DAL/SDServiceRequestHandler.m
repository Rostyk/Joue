...
//            [networkRequest setValue:@"text/xml" forHTTPHeaderField:@"Content-Type"];
...
#warning migrate to NSError mechanism
#warning no, seriousely, do it
#warning SDRequest will parse response from now on and fill SDResult object
#warning if anything goes wrong it'll provide nil result and meaningful error
#warning ok?
    // @discussion
    // we need to get status message if request failed
    // should we do it here?
    // or each response should deal with it?
    ...
    if (IS_SUCCESSFUL_HTTP_STATUS(networkResponse.statusCode))
    {
        ...
        if (!error && [jsonDict[@"status"] isEqualToString:@"error"])
        {
            ...
            if ([errors isKindOfClass:NSArray.class] && errors.count > 0)
            {
                for (NSString *errorString in errors)
                {
                    ...
                    {
                        [resultMessage appendFormat:errorString, @"/n"];
                    }
                }
            }
            
            result = [SDResult errorWithCode:401 message:resultMessage.length > 0 ? resultMessage : @"An Error occured"];
        }
        else
        {
